...
- **[multi_condo_framework_comparison.html](multi_condo_framework_comparison.html)** — build an ordered set of two to five named condominium records, keep project A as the reference, and compare the same project evidence and separate estate-context factor families in a scrollable matrix. Run `make multi-condo-framework-comparison`.
