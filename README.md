# Singapore Estate Liveability Framework

Versioned Python data/modeling pipeline for Singapore housing-estate Provision,
persona-relative Liveability, and tenure-segmented Value.

## Start here

- **[Architecture](docs/ARCHITECTURE.md)** — package boundaries, execution model, data governance, and publication.
- **[Data governance](docs/DATA_GOVERNANCE.md)** — input ownership, provenance, retention, and large-file policy.
...
## Python packages and compatibility CLIs

Canonical scoring models live under `sg_estate/domain/`; transactional
orchestration and master publication live under `sg_estate/application/`.
Historical paths under `models/` remain runnable compatibility CLIs:
...
- **[ingest_lta_rail.py](models/ingest_lta_rail.py)** — refreshes the code-line rail inventory and representative geometry from SHA-256-pinned, reviewed LTA/URA source snapshots; upstream byte changes stop for review. `onemap_geocode_mrt.py` remains a compatibility delegate.
- **12 `ingest_*.py` builders** — one per geospatial layer (jtc_industrial, air_quality, stewardship, tree_canopy, density, hawker v2, coastal, etc.). The canonical run consumes derived `tree_canopy`, `hdb_density`, `hawker_v2`, `coastal`, and `bca_permits` outputs.
...
See the [architecture guide](docs/ARCHITECTURE.md) or [Makefile](Makefile) for execution details.
...
- **[buyer_profile_table.html](buyer_profile_table.html)** — scenario-specific buyer screening with hard constraints, persona/horizon fit, tenure-separated Value and profile-local ranks. `min_value_band` passes a Value score up to `BORDERLINE_TOLERANCE` (0.05) below the band edge and records it in `borderline_flags`, because the regression-estimated Value score can cross an edge on a routine data correction; the other band minimums stay hard cut-offs. Refresh `data/outputs/buyer_profile_output.csv` with `python models/buyer_profile_model.py`, then rebuild the page with `python -m sg_estate.reporting.builders.buyer_profile`.
...
- **[framework_diagram.html](framework_diagram.html)** — generated architecture diagram (Inputs → Models → `data/outputs/master_output.csv`). Rebuild with `make framework-diagram`.
...
- **[Makefile](Makefile)** — test, transactional pipeline, report, and publication commands.
...
- **[tests/](tests/)** — pytest suite covering models, reproducibility, contracts, reports, and framework invariants. Markers: `integration` and `snapshot`.
