...
RUN:
    ...
    python -m sg_estate.domain.value --hdb hdb_resale.csv \
        --private ura_private.csv --scores provision_scores.csv
...
import numpy as np
import pandas as pd
from sg_estate.domain.aliases import ESTATE_TOWN_ALIAS, PRIVATE_DOMINANT_PROXIES
from sg_estate.domain.framework import MODEL_VERSION, band_label
from sg_estate.contracts import SCORE_BASE, VALUE, ContractError
...
# band() removed — use sg_estate.domain.framework.band_label.
...
def fit_segment(df, seg_name, scores):
    ...
    score_lookup = scores[["estate", "score"]].rename(
        columns={"estate": s["area_key"], "score": "_score"}
    )
    df = df.merge(
        score_lookup,
        on=s["area_key"],
...
def value_scores(resid_df, scores, original_estate_names):
    SCORE_BASE.validate(scores, source="value score input")
    duplicate_residuals = resid_df.duplicated(["estate", "segment"], keep=False)
    if duplicate_residuals.any():
        examples = (
            resid_df.loc[duplicate_residuals, ["estate", "segment"]]
            .drop_duplicates()
            .head(5)
            .to_dict("records")
        )
        raise ContractError(f"value residual input has duplicate keys: {examples}")
    ...
    df = resid_df.merge(scores, on="estate", how="left", validate="many_to_one")
...
def load_scores(path):
    sc = pd.read_csv(path)
    try:
        SCORE_BASE.validate(sc, source=path)
    except ContractError as exc:
        sys.exit(f"value_model contract error: {exc}")
    return sc
...
def main():
    ...
    try:
        VALUE.validate(result, source=a.out)
    except ContractError as exc:
        sys.exit(f"value_model contract error: {exc}")
...
#
# EXAMPLE RUN:
#   python -m sg_estate.domain.value --scores scores.csv --hdb hdb_resale.csv
#   python -m sg_estate.domain.value --scores scores.csv --hdb hdb.csv --private ura.csv
#
# WHAT YOU GET:
