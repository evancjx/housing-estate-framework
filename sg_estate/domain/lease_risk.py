...
RUN:
    python3 -m sg_estate.domain.lease_risk
"""
import argparse
import os
import sys
import pandas as pd
import numpy as np
from sg_estate.domain.aliases import ESTATE_TOWN_ALIAS
from sg_estate.domain.framework import MODEL_VERSION
from sg_estate.contracts import LEASE_RISK, ContractError
...
def run(hdb_path: str, estates_path: str, out_path: str) -> pd.DataFrame:
    ...
    try:
        LEASE_RISK.validate(result, source=out_path)
    except ContractError as exc:
        sys.exit(f"lease_risk_model contract error: {exc}")
...
def main():
    parser = argparse.ArgumentParser(description="Singapore Estate Lease Risk Model")
    parser.add_argument("--hdb", default=str(INPUT_DIR / "hdb_resale.csv"))
    parser.add_argument("--estates", default=str(INPUT_DIR / "estates.csv"))
    parser.add_argument("--out", default=str(OUTPUT_DIR / "lease_risk.csv"))
    args = parser.parse_args()
    run(args.hdb, args.estates, args.out)
