...
from sg_estate.contracts import EMPLOYMENT, ContractError
from sg_estate.paths import OUTPUT_DIR
...
def run(out_dir: str | Path = OUTPUT_DIR) -> dict[str, pd.DataFrame]:
    print("=" * 72)
    ...
    merged = df0[["estate","emp_score","emp_band"]].merge(
        df5[["estate","emp_score","emp_band"]],
        on="estate",
        suffixes=("_T0","_T5"),
        validate="one_to_one",
    ).merge(
        df15[["estate","emp_score","emp_band"]],
        on="estate",
        validate="one_to_one",
    ).rename(
    ...
    for horizon, frame in (("T0", df0), ("T5", df5), ("T15", df15)):
        try:
            EMPLOYMENT.validate(frame, source=f"employment_scores_{horizon}")
        except ContractError as exc:
            raise SystemExit(f"employment_model contract error: {exc}") from exc
    df0.to_csv(destination / "employment_scores_T0.csv", index=False)
    df5.to_csv(destination / "employment_scores_T5.csv", index=False)
    df15.to_csv(destination / "employment_scores_T15.csv", index=False)
    merged.to_csv(destination / "employment_trajectory.csv", index=False)
    print("\nWritten: employment_scores_T0/T5/T15.csv + employment_trajectory.csv")
    return {"T0": df0, "T5": df5, "T15": df15, "trajectory": merged}
