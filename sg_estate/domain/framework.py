#!/usr/bin/env python3
"""
Shared domain constants for the Singapore estate scoring pipeline.
...
NOTE: Alias maps are intentionally not defined here. They live in
sg_estate.domain.aliases as their single source of truth.
"""
...
def validate_framework_config() -> None:
    if set(PROVENANCE) != set(PROVISION_WEIGHTS):
        raise ValueError("PROVENANCE must cover each provision component exactly once")
    if set(PROVISION_WEIGHTS_PRIVATE) != set(PROVISION_WEIGHTS):
        raise ValueError("Private and HDB provision weights must use the same components")
