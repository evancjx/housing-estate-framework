...
Data flows from base geospatial and transactional data, through individual model scripts, to a final consolidated dataset `data/master_output.csv`.

## Code Layout
- `models/` — Python scoring models and ingesters.
- `scrapers/` — URA private transaction data scrapers.
- `data/` — Canonical input and output datasets.
...
### scrapers ↔ data/ura_private.csv
- Scrapers output raw District transaction data.
- Merger merges this into `data/ura_private.csv` maintaining schema consistency.
